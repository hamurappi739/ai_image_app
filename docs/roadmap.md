...
5. **Удаление изображений из аккаунта/backend** — после авторизации: реальное удаление из `generations` (не только локальная «Очистить»).
6. **RuStore Billing** — оплата пакетов генераций и фотосессий (100 ₽).
7. **Production cleanup** — удалить или защитить `/debug/*` endpoints; проверить CORS, секреты, RLS.

---

## Дальше (после MVP)

8. **Release** — публикация в RuStore, мониторинг, поддержка  
9. **Шаблоны / CMS** — каталог фотосессий с backend, A/B карточек  
10. **Watermark / preview quality** — preview vs полное качество после оплаты  
