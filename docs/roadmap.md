...
1. **Connect Gemini data URL result to Supabase Storage and save `public_url` in `generations`** — после реальной генерации вызывать `upload_generated_image_data_url`, записывать Storage URL в `generations.image_url` для Галереи.
2. **Подключить Storage для результатов фотосессий** — результаты обработки в Storage + запись в `generations`.
3. **Ручной тест Gemini (контролируемый)** — заранее пополнить баланс или подтвердить доступ к Gemini API/квотам, выполнить один тестовый `POST /generate` с коротким prompt.
