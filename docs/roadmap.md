...
| **UX: экран «Купить изображения»** | ✅ | Единая валюта **изображения**; 1 фото = 1 изображение; фотосессия = 3 изображения; варианты **1 / 3 / 9 / 20 / 50** фото (39–999 ₽); demo-покупки без списания денег; real RuStore — отдельный этап |
...
- **Купить:** единая валюта **изображения**; 1 фото = 1 изображение, фотосессия = 3 изображения; варианты **1 / 3 / 9 / 20 / 50** фото (39 / 99 / 249 / 499 / 999 ₽); demo mock-verify (деньги не списываются); real RuStore — future
...
## Монетизация и покупка изображений (актуальная экономика UI)

**Модель на экране «Купить изображения»:** покупаются **изображения** (без слова «кредиты» и без отдельного баланса фотосессий). **1 фото** = 1 изображение; **1 фотосессия** = 3 изображения. Варианты покупки: **1 / 3 / 9 / 20 / 50** фото за **39 / 99 / 249 / 499 / 999 ₽**. Demo-покупки через `PaymentService.purchasePackageDemo` — деньги не списываются; real RuStore payment — отдельный этап. Подробнее: [project_status.md](project_status.md).
