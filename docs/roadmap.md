...
**UX (следующие задачи):** расширить **«Как получить хороший результат»** на **«Создать»**; **идеи по категориям**; **backend prompts** (качество лиц); **backend balance model** — см. [app_design_strategy.md](app_design_strategy.md) и § **«Ближайший порядок работ»** ниже.
...
| 2 | **Стартовый баланс / 3 бесплатные генерации** — уведомление (*«Вам доступно 3 бесплатные генерации…»*) | план |
| 3 | **Показ баланса** в **«Профиль»** и кратко в **«Пакеты»** (*«Осталось: …»*; не «кредиты») | план |
| 4 | **Generation progress modal** + обратный отсчёт (**Создать** ~60 с, **Фотосессии** ~120 с) + затемнённый blocking overlay | ✅ |
| 5 | **Расширить «Как получить хороший результат»** на **«Создать»** + примеры описаний | **следующий UX** |
| 6 | **Готовые идеи по категориям** на **«Создать»** (Природа, Город, Портрет, …) | план |
| 7 | **Backend prompts — качество** (лица, без коллажа, one image; фотосессии — отдельные фото) | план |
| 8 | **Backend balance model** — `images_remaining` + `photoshoots_remaining`, free tier, списание | план |
| 9 | **RuStore payments** — верификация, начисление после покупки | план |

### Баланс и правила генерации (детализация)

**Старт:**

- Пользователь видит: *«Вам доступно 3 бесплатные генерации в разделе «Создать»»* или *«Осталось: 3 бесплатные генерации»*.
- **3 бесплатные** — только для обычной генерации (**«Создать»**), без оплаты.
...
### «Создать» — идеи и подсказки (план)

**Категории готовых идей:** Природа · Город · Люди/портрет · Дом/интерьер · Праздник · Реклама/товар · Соцсети/аватар.

**Примеры описаний** (блок «Как получить хороший результат»):
