...
4. **Реальная загрузка пользовательского фото** — image picker, отправка снимка на backend (фотосессии).
5. **Обработка фото через backend** — генерация **3 изображений** в выбранном стиле по загруженному фото.
6. **Платная фотосессия** — оплата **100 ₽** (RuStore) перед запуском платных стилей.
7. **Результаты фотосессии в Галерее** — сохранение трёх кадров в историю пользователя (backend + UI).
8. **Авторизация** — Supabase Auth, профиль, синхронизация баланса генераций.
9. **Сохранение истории на аккаунт** — `GET /generations` по auth user id; без `TEST_USER_ID`.
10. **Удаление изображений из аккаунта/backend** — после авторизации (не только локальная «Очистить»).
11. **RuStore Billing** — пакеты генераций на вкладке «Пакеты».
12. **Production cleanup** — удалить или защитить `/debug/*` endpoints; CORS, секреты, RLS.
