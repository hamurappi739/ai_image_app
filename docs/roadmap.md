...
**UX (следующие задачи):** см. [app_design_strategy.md](app_design_strategy.md) — **баланс пользователя** (изображения + фотосессии), curated-примеры, backend **«Создать»** / **«Своя фотосессия»**, RuStore.
...
| # | Задача | Статус |
|---|--------|--------|
| 1 | **Обновить Flutter вкладку «Пакеты»** — смешанные пакеты, переключатель, «Своя сумма», layout | ✅ |
| 2 | **UI «Своя сумма»** — ввод 200–100 000 ₽, выбор фотосессий, расчёт | ✅ |
| 3 | **Packs tab contextual help** — `PacksHelpDialog`, кнопка **«Помощь»** | ✅ |
| 4 | **Баланс пользователя (frontend + backend)** — *«Осталось: X изображений и Y фотосессий»*; не «кредиты» | **следующий** |
| 5 | **Показ баланса в «Профиль»** — основной экран остатка | план |
| 6 | **Краткий баланс во вкладке «Пакеты»** — сверху, перед пакетами | план |
| 7 | **Баланс в «Создать» и «Фотосессии»** (кратко) — после основного этапа | позже |
| 8 | **Backend balance model** — `images_remaining` + `photoshoots_remaining` (не только `paid_credits`) | план |
| 9 | **RuStore payment verification** — подтверждение оплаты, webhook / idempotency | план |
| 10 | **Списание после оплаты** — изображения при `POST /generate`, фотосессии при платной фотосессии; начисление после покупки | план |

**Оплата и RuStore пока не подключены.** UI пакетов готов как **placeholder** («Оплата скоро»).

### Следующий этап: баланс пользователя (детализация)

1. **Backend:** хранить и отдавать остаток **изображений** и **фотосессий** (миграция / API, например `GET /profile/balance` или расширение профиля).
2. **Профиль (Flutter):** основной блок *«У вас осталось: N изображений и M фотосессий»*.
3. **Пакеты (Flutter):** компактная строка/карточка с тем же балансом сверху вкладки.
4. **Позже:** краткий остаток на **«Создать»** и **«Фотосессии»** перед генерацией.
5. **После баланса:** RuStore → подтверждение оплаты → начисление по выбранному пакету / «Своей сумме».
