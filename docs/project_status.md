...
- **Intro-блок (реализовано):** подзаголовок, **«1 фотосессия = до 3 изображений»**, **баланс фотосессий**, loading state; предупреждение при нулевом балансе → **Пакеты**.
- **Каталог карточек (реализовано, demo-ready):** gradient preview, **название**, **описание**, бейджи **«Бесплатно»** / **«100 ₽»**, **«до 3 фото»**, **рекомендация** («Для аватара», «Для работы» и т.д.); платные — диалог *«Оплата скоро…»* без генерации. **Реальные curated-примеры** — позже.
...
- **Layout:** адаптивная сетка (1 / 2 / 3 колонки); одинаковая высота карточек; Chrome + Android без overflow.
