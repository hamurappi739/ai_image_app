...
- **Модальное ожидание** при генерации по описанию — см. § **«Ожидание генерации»** (~60 с).
...
| 9 | **Расширить «Как получить хороший результат»** + примеры описаний | план |
| 10 | **Идеи по категориям** на «Создать» | план |
| 11 | **Backend prompts — качество лиц и изображений** | план |
| 12 | **Оплата** — backend balance model + RuStore | план |
...
- Следующие шаги (см. [roadmap.md](roadmap.md)): **мин. пополнение 10 ₽** → **3 бесплатные генерации + баланс в UI** → **идеи по категориям** + **подсказки «Создать»** → **backend prompts (качество)** → **balance model + RuStore** → curated-примеры → backend **«Создать»** / **«Своя фотосессия»**
