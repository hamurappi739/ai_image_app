...
- **Актуальный UI (UX-redesign):** навигация через **burger/drawer** слева сверху; главная — welcome-экран; пользовательский путь **от простого к сложному**: **1) Фото по шаблону → 2) Фотосессии → 3) Свой запрос**. См. [navigation_redesign_plan.md](navigation_redesign_plan.md).
...
- **Debug APK на физическом телефоне (✅, UX-redesign):** `flutter build apk --debug --dart-define=API_BASE_URL=http://192.168.31.242:8000` → `adb install -r`; backend в **demo mock-режиме** (`ENABLE_CREDIT_CONSUMPTION=true`, `IMAGE_PROVIDER=mock`, `ENABLE_PHOTOSHOOT_GENERATION=true`, `PHOTOSHOOT_OUTPUT_COUNT=3`) с **`--host 0.0.0.0 --port 8000`**. Проверены: **Главная**, drawer, **Фото по шаблону** → **Свой запрос**, фото + генерация, **Фотосессии**, **Купить**, **Готовые фото**, **Помощь** (без overflow). См. [demo_release_checklist.md](demo_release_checklist.md) § **D**.
...
**Основная навигация (русский UI):** **burger/drawer** слева сверху. Нижняя tab bar **убрана**. Состояние экранов — `IndexedStack` в `MainShell`.
...
| **Фотосессии** | вкладка | Популярные / другие стили; блок «Создать свой образ» сверху |
| **Свой запрос** | **Создать** | Фото → описание → «Создать фото»; прямой вход из меню — пустой экран |
| **Готовые фото** | **Галерея** | История генераций; empty/loading/error states |
| **Купить** | **Пакеты** | Баланс, наборы, «Своя сумма»; mock top-up через backend |
| **Профиль** | вкладка | Вход / регистрация / баланс |
| **Помощь** | — | Hub с темами по каждому разделу |

**Пользовательский путь:** **1) Фото по шаблону** (самый простой) → **2) Фотосессии** → **3) Свой запрос** (своя идея). Подробнее: [navigation_redesign_plan.md](navigation_redesign_plan.md).
...
| **Показ баланса** | **Профиль** / **Купить** / **Свой запрос** — бесплатные, фото, фотосессии; **не** «кредиты» | ✅ |
...
- **6 шаблонов** с готовым `requestDescription` (деловой портрет, соцсети, зимний, резюме, красивый портрет, товар).
- **«Выбрать»** → переход в **Свой запрос** с автозаполнением поля описания + SnackBar *«Описание уже добавлено. Осталось выбрать фото.»*
...
- **Баннер баланса (реализовано):** `_CreateBalanceInfoCard` — free/paid из `GET /balance`; демо-режим; подсказка *«Бесплатные генерации закончились — используйте фото из баланса»* при переходе на paid.
...
- **Intro-блок (реализовано):** подзаголовок, **«1 фотосессия = 3 изображения»**, **баланс фотосессий**, loading state; предупреждение при нулевом балансе → **Пакеты**.
- **Каталог карточек (реализовано, demo-ready):** gradient preview, **название**, **описание** (2 строки в карточке, полнее в модалке), бейджи **«Бесплатно»** / **«100 ₽»**, **«3 фото»**, **рекомендация**; платные — диалог *«Оплата скоро…»* без генерации.
...
- **Группировка стилей:** **«Популярные фотосессии»** (4 стиля) + **«Другие стили»** (остальные).
...
- Заголовок: **«Готовые фото»**; подзаголовок: *«Здесь будут храниться фото, которые вы создали.»*
- При старте: **`GET /generations`**; состояния **загрузки** (*«Загружаем готовые фото…»*), **ошибки** (*«Не удалось загрузить…»* + **«Повторить»**), **пусто** (*«Пока здесь пусто»* + кнопки **«Фото по шаблону»** / **«Фотосессии»**).
- **Одиночные фото:** превью, описание, подпись **«Фото»**, кнопка **«Открыть»**; модалка — **«Готовое фото»**, **«Скачать»**, **«Скрыть из готовых фото»**, **«Закрыть»**.
- **Фотосессии:** карточка с заголовком стиля, бейдж **«3 фото»**, мини-сетка; **«Открыть фотосессию»**; модалка — **«Фотосессия»**, скачать каждое фото, **«Скрыть фотосессию»**.
- **Группировка:** по **`photoshoot_id`**; legacy записи — отдельные карточки.
- **Локальное скрытие** и **«Очистить»** — только на устройстве; backend не удаляется.
...
| Flutter Android **физический телефон** (UX-redesign) | ✅ **debug APK** (`API_BASE_URL=http://192.168.31.242:8000`); drawer, шаблоны → **Свой запрос**, фото + генерация, все разделы, помощь без overflow; backend demo mock (раздел A в [demo_release_checklist.md](demo_release_checklist.md)) |
