...
- **`POST /generate`** через `ApiService` — сейчас только **текст → одно изображение**.
- **План:** **фото + описание → одно изображение** (не фотосессия); см. [app_design_strategy.md](app_design_strategy.md) §10.
...
### Продуктовые UX-требования (план, не реализовано)

Согласовано с product owner; детали — [app_design_strategy.md](app_design_strategy.md):

| Приоритет | Требование |
|-----------|------------|
| 1 | **First-run onboarding** — 5 экранов, «Далее» / «Пропустить», один раз |
| 2 | **Карточки фотосессий** — пример результата, цена, «3 фото», описание; рекомендации по исходному фото в sheet |
| 3 | **«Создать»:** фото + **описание** → **одно** изображение |
| 4 | **«Своя фотосессия»** — фото + свои пожелания + текст-помощник |
| 5 | **Оплата** платных фотосессий |
...
- Следующие шаги (UX, см. [roadmap.md](roadmap.md)): **onboarding** → **карточки фотосессий** → **фото+описание на «Создать»** → **своя фотосессия** → **оплата**; затем product mode **`PHOTOSHOOT_OUTPUT_COUNT=3`**
