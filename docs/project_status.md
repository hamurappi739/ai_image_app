...
| 5 | **Помощь для «Пакетов»** — после обновления UI пакетов | план |
| 6 | **Оплата** — RuStore + backend balance (изображения + фотосессии) | план |
| — | **Экономика пакетов** (10 ₽ / изображение, 100 ₽ / фотосессия, смешанные пакеты) — в docs | 🔶 UI legacy |
...
**Будущий режим «Своя сумма» (UX, не реализовано):** сумма **200–100 000 ₽** → пользователь выбирает число фотосессий → остаток ÷ 10 = обычные изображения. Пример **1000 ₽**, **8** фотосессий → **8** фотосессий + **20** изображений; при **0** фотосессий → **100** изображений.

**Статус реализации:** экономика **зафиксирована в документации**; Flutter вкладка **«Пакеты»** пока показывает **старый** UI (199 / 499 / 1199 ₽, только «изображения»); **RuStore и списание баланса — не подключены**.

### Пакеты

- **Целевая модель:** см. **«Продуктовая экономика»** выше (смешанные пакеты 199 / 499 / 999 ₽, переключатель «Только изображения», позже «Своя сумма»).
- **Сейчас во Flutter (legacy UI):** 199 ₽ → 25 изображений; 499 ₽ → 100; 1199 ₽ → 250; кнопка **«Скоро»**; **RuStore** не подключён.
- **Пользовательский баланс (цель):** *«У вас осталось: 12 изображений и 2 фотосессии»* — не «кредиты».
...
**Использовать для пользователя:** описание, идея, **изображение** / **изображений**, **фотосессия** / **фотосессии**, **пакеты**; баланс: *«осталось: N изображений и M фотосессий»*.
...
- Следующие шаги (UX/монетизация, см. [roadmap.md](roadmap.md)): **вкладка «Пакеты»** (смешанные пакеты) → curated-примеры → backend **«Создать»** / **«Своя фотосессия»** → **баланс изображений+фотосессий** → **RuStore** → product mode **`PHOTOSHOOT_OUTPUT_COUNT=3`**
