...
- Следующие шаги (UX, см. [roadmap.md](roadmap.md)): **реальные примеры на карточках** → **фото+описание на «Создать»** → **своя фотосессия** → **art direction** → **помощь для «Пакетов»** → **RuStore / оплата**; затем product mode **`PHOTOSHOOT_OUTPUT_COUNT=3`**
