...
- **Группировка фотосессий:** записи с одинаковым non-null **`photoshoot_id`** → **одна карточка** «**Фотосессия · &lt;стиль&gt;**» + бейдж **«N фото»** + сетка preview; legacy / обычные записи с **`photoshoot_id=null`** — **отдельные карточки**.
- **Просмотр (Gallery 2.0):** клик по карточке → dialog с крупным изображением или фотосессией (сетка **3 фото**); **«Скачать»** (web: download / новая вкладка); **«Скрыть из Галереи»** / **«Скрыть фотосессию из Галереи»** — только локально, с подтверждением; backend и Supabase Storage **не удаляются**.
- **Локальное скрытие:** `hiddenGalleryImageKeys` / `hiddenPhotoshootIds` в памяти приложения; после перезапуска скрытые элементы могут вернуться из **`GET /generations`** — ожидаемо на текущем этапе.
- Скрывает служебные **debug**-описания в UI.
- Новые результаты сессии — **сверху** после **Создать** или фотосессии.
- **«Очистить»** — только локальный список + сброс скрытых id; **Supabase не удаляется** (SnackBar: «Галерея очищена на этом устройстве»).
- **Empty state:** «В Галерее пока пусто» + призыв создать изображение или фотосессию.
...
- **Demo-ready UI (Flutter):** hero **«Ваш баланс»** — крупно **Изображения / Фотосессии / Бесплатные (Z из 3)**; loading / *«Не удалось загрузить баланс. Попробуйте позже.»*; плашка **«Демо-режим: списание отключено.»** при `consumption_enabled=false`.
- **Переключатель:** **«С фотосессиями»** / **«Только изображения»** с пояснением под switch; готовые пакеты **199 / 499 / 999 ₽** с выгодой (*«Для первого теста»* / *«Оптимальный вариант»* / *«Для активного использования»*); **«Популярно»** на **499 ₽**.
- **«С фотосессиями»:** 199 ₽ → 1 фотосессия + 9 изображений; 499 ₽ → 3 + 19; 999 ₽ → 8 + 19.
- **«Только изображения»:** 199 ₽ → 19; 499 ₽ → 49; 999 ₽ → 99.
- **Кнопки:** **«Выбрать пакет»** / **«Пополнить баланс»** → диалог **«Оплата скоро появится»** (*«Сейчас это демонстрационный режим…»*, **«Понятно»**); **RuStore / реальная оплата — не подключены**.
