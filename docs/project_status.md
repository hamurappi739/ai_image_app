...
- **Актуальный UI (UX-redesign):** burger/drawer; welcome-**Главная**; **баланс в меню и шапке**; пути **Фото по шаблону → Свой запрос → Готовые фото** и **Фотосессии → Готовые фото**; **Купить** при нехватке баланса; мягкие подсказки при нуле. См. [navigation_redesign_plan.md](navigation_redesign_plan.md).
...
**Основная навигация:** **burger/drawer** слева сверху; **баланс** в верхней части drawer и компактно в шапке (**Фото: N**). Нижняя tab bar **убрана**. `IndexedStack` в `MainShell`.
...
| **Показ баланса** | **Drawer**, **шапка**, **Профиль**, **Купить**, **Свой запрос**; подписи **Фото / Фотосессии / Бесплатные** | ✅ |
...
**Flutter:** баланс в **drawer**, **шапке**, **Профиль**, **Купить**, **Свой запрос**; обновление после генерации / покупки; **402** → мягкий диалог с переходом в **«Купить»** (реальный RuStore — не подключён).
...
- Шаблоны **сгруппированы по категориям** (например: для себя, для работы, для семьи, для продажи); визуальные **placeholder** на карточках.
- **«Выбрать»** → **Свой запрос** с автозаполнением описания + SnackBar *«Описание добавлено. Осталось выбрать фото.»*
