...
- Следующие шаги (UX, см. [roadmap.md](roadmap.md)): **реальные примеры на карточках** → **backend для фото+описание на «Создать»** → **backend для «Своей фотосессии»** → **art direction** → **помощь для «Пакетов»** → **RuStore / оплата**; затем product mode **`PHOTOSHOOT_OUTPUT_COUNT=3`**
