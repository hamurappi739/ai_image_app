...
| **Идеи на «Создать»** | Готовые идеи по **категориям** (Природа, Город, Портрет, …) | план |
| **Время генерации** | Примерное время в UI (**«Создать»**, **«Фотосессии»**) — *«20–60 сек»* / *«до минуты»* | план |
| **Стартовый баланс** | **3 бесплатные генерации** в **«Создать»**; понятное сообщение пользователю | план |
...
| **«Как получить хороший результат»** | Расширить блок: больше подсказок + **примеры описаний** под полем | план |
...
- Ввод **описания**, быстрые идеи (**сейчас** — один общий список chips; **план** — категории: Природа, Город, Люди/портрет, Дом/интерьер, Праздник, Реклама/товар, Соцсети/аватар).
- Блок **«Как получить хороший результат»** (**сейчас** — краткий; **план** — расширить + примеры описаний под полем).
- **План UI:** примерное время генерации (*«Создание может занять 20–60 секунд»* / *«Обычно занимает до минуты»*) перед/во время генерации.
- **План:** стартовое сообщение *«Вам доступно 3 бесплатные генерации в разделе «Создать»»*; баланс *«Осталось: N бесплатных генераций»* → позже смешанный баланс изображений и фотосессий.
- **План:** после исчерпания бесплатных — генерация только при платном балансе; frontend объясняет пополнение (**Пакеты**).
...
- **План UI:** примерное время генерации в bottom sheet / во время ожидания (как на **«Создать»**).
...
| 7 | **3 бесплатные генерации** — сообщение + учёт баланса | план |
| 8 | **Показ баланса** — Профиль + Пакеты | план |
| 9 | **Примерное время генерации** — Создать + Фотосессии | план |
| 10 | **Расширить «Как получить хороший результат»** + примеры описаний | план |
| 11 | **Идеи по категориям** на «Создать» | план |
| 12 | **Backend prompts — качество лиц и изображений** | план |
| 13 | **Оплата** — backend balance model + RuStore | план |
...
- Следующие шаги (см. [roadmap.md](roadmap.md)): **мин. пополнение 10 ₽** → **3 бесплатные генерации + баланс в UI** → **время генерации** → **идеи по категориям** + **подсказки «Создать»** → **backend prompts (качество)** → **balance model + RuStore** → curated-примеры → backend **«Создать»** / **«Своя фотосессия»**
