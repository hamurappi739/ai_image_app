...
| **Купить** | Пакеты 199/499/999 ₽, «Своя сумма», баннер баланса | ✅ (demo top-up) |
...
| Правило | Значение |
|---------|----------|
| Бесплатные фото | **3** на старте (`free_generations_used` / лимит в профиле) |
| Цена 1 фото | **10 ₽** |
| Цена 1 фотосессии | **100 ₽** |
| Состав фотосессии | **3 фото** (`PHOTOSHOOT_OUTPUT_COUNT=3`, один `photoshoot_id`) |
| Списание фотосессии | **1** единица `paid_photoshoots` после успеха |

**Demo / mock payments:** ✅ — вкладка **Купить** в development вызывает backend mock-verify; баланс обновляется в drawer, Профиле и на экранах. Frontend **не** начисляет баланс сам.
