...
- Если backend photoshoot generation **включён** (`ENABLE_PHOTOSHOOT_GENERATION=true`), успешный ответ с `image_urls` добавляет результаты в **Галерею** (описание «Фотосессия: …»), показывает «Фотосессия готова» и переключает на вкладку «Галерея»
- Если backend **safety switch выключен** (`ENABLE_PHOTOSHOOT_GENERATION=false`, по умолчанию), backend возвращает **501** — UI показывает мягкое placeholder-сообщение «Обработка фото будет добавлена позже»
- Платные фотосессии пока не отправляют фото на backend; при выбранном фото — «Оплата будет добавлена позже»
...
- После генерации или успешной фотосессии новый результат сразу добавляется **сверху** (локально)
