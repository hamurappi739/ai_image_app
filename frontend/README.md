...
- Для бесплатного сценария после выбора фото вызывается backend placeholder `POST /photoshoots/generate`
- Ответ `501` обрабатывается как ожидаемая заглушка с мягким сообщением для пользователя
- Фото хранится только локально в состоянии окна и не отправляется на backend
